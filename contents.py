...
class Player(Person):
    def __init__(self, name, sex, att, defence, luck, maxhp):
        ...
        self.coin = 40
        self.level = 1
        self.skill = [1,1,1,3,3]
        self.item = [0,0,0,0,0]

    def poison(self, target, turn):
        if self.skill[0] == 0:
            return
        if turn < 3:
            return True
    ...
    def def_decrease(self, target):
        ...
        return f"상대 방어력 {100*percent[self.skill[1]]}% 감소! "
    ...
    def att_decrease(self, target):
        ...
        return f"상대 공격력 {100*percent[self.skill[2]]}% 감소! "
    ...
    def reflect(self, target, targetDmg):
        ...
        if random.random() < percent[self.skill[3]]:
            ...
            target.hp -= dmg
            return f"반사 성공! 체력 회복! 데미지 {int(dmg)} "
        else: return "반사를 실패했습니다.. "
